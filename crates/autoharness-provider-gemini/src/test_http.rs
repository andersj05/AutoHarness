...
pub(crate) async fn spawn(responses: Vec<ResponseSpec>) -> (Url, JoinHandle<Vec<RecordedRequest>>) {
    ...
    let base_url = Url::parse(&format!("http://{address}/")).expect("fixture base URL");
...
pub(crate) async fn spawn_slow_sse(
    ...
    let task = tokio::spawn(async move {
        ...
        socket
            ...
            .await
            .expect("write fixture chunk size");
        socket
            .write_all(&initial_event)
            .await
        ...
        socket.flush().await.expect("flush fixture event");
...
async fn read_request(socket: &mut TcpStream) -> RecordedRequest {
    ...
    let header_end = loop {
        ...
        assert!(bytes.len() < MAX_REQUEST_BYTES, "fixture request too large");
    ...
    let header_text = std::str::from_utf8(&bytes[..header_end]).expect("UTF-8 request headers");
...
async fn write_response(socket: &mut TcpStream, response: ResponseSpec) {
    ...
    socket.flush().await.expect("flush fixture response");
}
