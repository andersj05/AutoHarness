...
- On 2026-08-25, the Settings Providers workspace became a keyboard-first provider catalog with explicit catalog and saved-connection focus, `Esc` return to Settings, API-key starters for Google AI Studio, OpenAI or Codex, OpenRouter, Groq, Mistral AI, and custom compatible endpoints, honest account-key messaging, and no claim of unsupported OAuth account sign-in; the inline command list now preserves the chat panel border, 55 focused TUI tests and the real Windows provider PTY journey passed.
