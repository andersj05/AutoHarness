...
- On 2026-08-26, `feat/fix-provider-login` fixed the Windows PowerShell `-Command` argument handoff that prevented `codex login` from launching, reduced Codex setup to browser sign-in followed by one save-and-check action, made saved Codex connections reopen the same sign-in wizard, and replaced the ambiguous connected-account detail pane with provider, use, last-check, sign-in, ownership, default-model, and thinking labels.
