...
- On 2026-08-25, Providers became a connected-account list, Agents gained a provider -> compatible model -> provider-default thinking workflow, first accepted prompts atomically name sessions from their redacted opening line, and the `codex_cli` provider bridges a user-owned official Codex CLI subscription login through read-only ephemeral JSONL execution without importing subscription credentials; 413 workspace tests, strict Clippy, formatting, documentation links, and the real Windows account-and-agent PTY journey passed.
