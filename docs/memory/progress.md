...
| 3.5. Terminal release hardening | Implemented | Six real-PTY release scenarios, serial cross-platform CI routing, locked-vault and corrupt-cache recovery, provider-matched environment credentials, complete structural live-probe definitions, content-free monotonic terminal markers, a PTY latency runner, and the terminal release checklist pass local compile, unit, integration, baseline, benchmark, documentation, and real Windows router-terminal smoke validation; release-candidate PTY, live-provider, and reference-machine evidence remains open |
...
- The dedicated PTY scenarios compile locally, but their Windows execution awaits the release-candidate CI job because this development harness cannot start a nested ConPTY.
