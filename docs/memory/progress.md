...
| 3.9. Terminal product validation | Implemented locally; evidence pending | This branch adds zero-shell onboarding, first-draw boot animation, settings-owned credential entry without launch interruption, geometry-derived semantic mouse actions, command parity for `/settings`, `/models`, `/sessions`, `/profile`, `/connect-api-key`, `/retry`, `/cancel`, `/search`, and `/toggle-tools`, footer-free compact prompt rendering, default neon styling, and 385 workspace tests; the full release-candidate evidence matrix remains open |
