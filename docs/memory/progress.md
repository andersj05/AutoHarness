...
| 3.9. Terminal product validation | Implemented locally; evidence pending | This branch adds zero-shell onboarding, a local user-profile dialog, real mouse capture and semantic click actions, responsive Settings action visibility, provider/profile default context, command-palette labels, contextual-help parity, bounded paste editing, and route/session context; focused TUI, workspace, and selected Windows PTY checks pass while the full release-candidate evidence matrix remains open |
...
- This branch passes workspace formatting, strict Clippy, 345 workspace tests across 46 suites, 116 focused TUI tests, two ignored visual-review renders, and the relative Markdown-link check.
