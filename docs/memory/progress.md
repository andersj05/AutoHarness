...
| 3.9. Terminal product validation | Implemented locally; evidence pending | This branch adds zero-shell onboarding, first-draw boot animation, settings-owned credential entry, geometry-derived mouse actions, live slash command filtering, `/provider` setup, `/models` default-model persistence, Aurora and Ember themes, footer-free compact prompt rendering, and 389 workspace tests; the full release-candidate evidence matrix remains open |
