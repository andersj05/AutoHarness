...
| 3.9. Terminal product validation | Implemented locally; evidence pending | This branch adds zero-shell onboarding, first-draw boot animation, settings-owned credentials, geometry-derived mouse actions, eight-row inline slash commands, a borderless prompt metadata bar with safe workspace and Git context, `/provider` setup, `/models` default persistence, Aurora and Ember themes, and 390 workspace tests; the full release-candidate evidence matrix remains open |
