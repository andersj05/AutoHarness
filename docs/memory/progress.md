...
| 3.6. Local profile and provider connection center | Planned | Add a full-screen local profile and provider-management surface with multiple named connections, distinct operating-system-vault credentials, safe testing, source provenance, and complete in-terminal lifecycle actions |
...
- The current `Ctrl+,` settings surface is a read-only provider and credential summary; users cannot create or edit profiles, save distinct provider keys, test connections, or complete credential lifecycle actions inside the terminal.
- The current TUI is organized around independent overlays rather than primary routes, and it has no local user profile summary, full settings editor, theme selection, high-contrast or no-color mode, ASCII mode, or single-column accessibility presentation.
...
Phase 3.5 must pass green baseline and dedicated PTY pull-request gates on Windows, macOS, and Linux and merge to `dev`.
Phase 3.6 then exits only when a fresh user can manage multiple provider profiles and distinct operating-system-vault credentials completely inside the terminal, with cross-store recovery, sentinel coverage, and platform vault smoke evidence.
