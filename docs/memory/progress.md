...
| 3.8. Personalization and accessibility | In progress | Extend the existing Settings route and typed resolver with an in-terminal settings workspace, provenance and reset controls, typed local-profile preferences, themes, no-color and high-contrast modes, reduced motion, ASCII mode, density, and single-column presentation |
