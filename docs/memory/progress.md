...
| 3.9. Terminal product validation | Implemented locally; evidence pending | This branch adds zero-shell onboarding, a local user-profile dialog, mouse capture and semantic click actions for routes, chat, sessions, profiles, confirmations, picker rows, and palette rows, responsive Settings action visibility, provider/profile default context, contextual-help parity, bounded paste editing, and route/session context; 117 focused TUI tests and 346 workspace tests pass while the full release-candidate evidence matrix remains open |
...
- This branch passes workspace formatting, strict Clippy, 346 workspace tests across 46 suites, 117 focused TUI tests, two ignored visual-review renders, and the relative Markdown-link check.
