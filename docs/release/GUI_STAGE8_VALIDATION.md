...
Frontend type checking, the 116-test GUI suite, production frontend build, and the 14-test Python tooling suite pass locally.
...
Hosted EdgeDriver reported missing `DevToolsActivePort` despite a ready native renderer, including with matching explicit profile and runtime directories.
The Windows test launcher now owns the isolated app process and uses an explicit loopback debugger connection instead of that discovery file.
This passes the complete local native journey, with focused attachment and failure-cleanup regression tests; the packaged application gains no debug port or new capability during ordinary launch.
