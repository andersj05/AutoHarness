  ...
  return <main className="routeWorkspace memoryWorkspace" id="main-content" tabIndex={-1} aria-label="Memory workspace">
    ...
    <div className="memoryColumns" aria-busy={!ready}>
      ...
      <section className="memoryDetail" aria-label="Memory detail">
        ...
        </> : <div className="memoryEmpty"><Icon name="memory" size={28} /><h2>{ready ? "Select a memory" : "Loading memory"}</h2><p>Revision details, provenance, evidence, and admissions appear here.</p></div>}
      </section>
    </div>
    <p role="status" className="memoryStatus">{message}</p>
    {review && !blocked ? createPortal(<Dialog title={titles[review.action]} eyebrow="Exact memory scope" onClose={() => { if (!busy) setReview(undefined); }} description={review.action === "approve" ? "Approve this version for use in future conversations." : review.action === "delete" ? "Erase retained memory content and evidence excerpts. Audit identities remain. Existing user-owned exports are not removed. Export first if you need a copy." : review.action === "retract" ? "Prevent this exact revision from admission into future turns. Existing admission history remains auditable." : review.action === "import" ? "Enter a text file path within your workspace (up to 16 KiB). You can review the imported memory before approving it." : review.action === "export" ? "Write a standalone JSON export beside the database. It contains retained memory content and audit history." : "Save this change to memory."}
